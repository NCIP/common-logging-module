...
import gov.nih.nci.logging.webapp.util.Utils;
...
public class SummaryDaoJdbc implements Constants
{
	...
	private String generateSQL(QueryForm q)
	{
		...
		sql.append("SELECT L.OID,L.APPLICATION,L.SERVER,L.PRIORITY," + "L.THREAD,L.CREATED_ON, L.USERNAME, L.SESSION_ID, L.MSG, L.NDC, L.THROWABLE FROM LOGTAB L WHERE ");
		...
		{
			sql.append(" AND L.USERNAME = '" + q.getUser() + "'");

		}

		if (Utils.hasValue(q.getSessionid()))
		{
			sql.append(" AND L.SESSION_ID = '" + q.getSessionid() + "'");
